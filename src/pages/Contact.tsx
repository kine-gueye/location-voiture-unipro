...
const Contact = () => {
  ...
  const [form, setForm] = useState({
    nom: "", email: "", telephone: "", ville: "", message: "", service: ""
  });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center animate-bounce-in p-10 bg-white rounded-3xl shadow-xl max-w-md">
          ...
          <p className="text-gray-500">Nous vous répondrons dans les 24h. Merci {form.nom} !</p>
          <button
            onClick={() => { setSent(false); setForm({ nom: "", email: "", telephone: "", ville: "", message: "", service: "" }); }}
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 py-16 px-4">
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-gray-500 max-w-xl mx-auto">
          Une question ? Une réservation ? Remplissez le formulaire et nous vous répondons rapidement.
        </p>
      </div>
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={i} className="bg-white rounded-2xl p-4 shadow-sm flex items-center gap-4 hover:shadow-md hover:scale-105 transition-all duration-300">
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="relative">
                <label className={`absolute left-4 transition-all duration-300 pointer-events-none ${focused === "nom" || form.nom ? "top-1 text-xs text-blue-500" : "top-4 text-sm text-gray-400"}`}>
                  Nom complet *
                </label>
                <input
              ...
              <div className="relative">
                <label className={`absolute left-4 transition-all duration-300 pointer-events-none ${focused === "email" || form.email ? "top-1 text-xs text-blue-500" : "top-4 text-sm text-gray-400"}`}>
                  Email *
                </label>
                <input
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="relative">
                <label className={`absolute left-4 transition-all duration-300 pointer-events-none ${focused === "telephone" || form.telephone ? "top-1 text-xs text-blue-500" : "top-4 text-sm text-gray-400"}`}>
                  Téléphone
                </label>
                <input
              ...
              <div className="relative">
                <label className={`absolute left-4 transition-all duration-300 pointer-events-none ${focused === "ville" || form.ville ? "top-1 text-xs text-blue-500" : "top-4 text-sm text-gray-400"}`}>
                  Ville
                </label>
                <input
                  type="text" name="ville" value={form.ville}
                  onChange={handleChange}
            ...
            {/* Service */}
            <div className="relative">
              <label className="block text-xs text-gray-400 mb-1 ml-1">Type de service *</label>
              <select
            ...
            {/* Message */}
            <div className="relative">
              <label className={`absolute left-4 transition-all duration-300 pointer-events-none ${focused === "message" || form.message ? "top-1 text-xs text-blue-500" : "top-4 text-sm text-gray-400"}`}>
                Votre message *
              </label>
              <textarea
