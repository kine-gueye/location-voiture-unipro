    ...
    if (selectedSpace) {
      const updatedSpaces = spaces.map(s => 
        s.id === selectedSpace.id 
          ? { ...s, isOccupied: true, carPlate: formData.get('plate') as string, owner: formData.get('owner') as string }
          : s
      );
          ...
          >
            {space.isOccupied ? (
              <div className="flex flex-col items-center">
                <img src={toyotaImg} alt="Toyota" className="w-16 h-10 object-contain mb-1" />
            ...
            <span className="absolute top-1 left-2 text-[10px] font-black text-[#1e293b]">#{space.id}</span>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="bg-[#1e293b] p-4 text-white flex justify-between items-center">
              <h2 className="font-bold">Enregistrer un véhicule - Place #{selectedSpace.id}</h2>
              <button onClick={() => setSelectedSpace(null)}><X size={20} /></button>
            </div>
            ...
            <form onSubmit={handleRegisterCar} className="p-6 space-y-4">
              <div>
                ...
                <input name="owner" type="text" required className="w-full border border-gray-300 p-2 rounded-lg focus:ring-2 focus:ring-[#3b82f6] outline-none" placeholder="Ex: Youssouf Bah" />
              </div>
              
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Numéro de Plaque</label>
                <input name="plate" type="text" required className="w-full border border-gray-100 p-2 rounded-lg focus:ring-2 focus:ring-[#3b82f6] outline-none" placeholder="Ex: DK-1234-AB" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Modèle du véhicule</label>
              ...
              <button type="submit" className="w-full bg-[#3b82f6] text-white font-bold py-3 rounded-lg hover:bg-[#1e293b] transition-colors mt-4">
